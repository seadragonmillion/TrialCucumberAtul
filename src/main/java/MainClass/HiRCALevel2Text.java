...
public class HiRCALevel2Text {
	...
	public void step2verify317 (WebDriver driver, SoftAssertions softly) throws Exception {
		...
		softly.assertThat(desc).as("test data").contains("If the triggering event is caused by equipment, the user can do a preliminary assessment on the equipment's design spec qualification, testing and maintenance. It is recommended to use MainClass.EiRCA™ to do a troubleshooting specifically for equipment failures.");
		//Click on Description
		wait.until(ExpectedConditions.visibilityOfElementLocated(hirca.DescriptionPlusSign)).click();
		//3.17.1 question
